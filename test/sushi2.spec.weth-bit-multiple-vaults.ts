...
describe("SushiStakingV2Strategy - WETH-BIT multiple vaults", async function () {
  ...
  describe(`WETH-BIT - invest first vault`, async function () {
    shouldBehaveLikeVaultDuringInvestments(0);
    describe(`WETH-BIT - invest second vault`, async function () {
      shouldBehaveLikeVaultDuringInvestments(1);
      describe(`WETH-BIT vault behaviour - harvest and redeeem first vault`, async function () {
        shouldBehaveLikeDualRewardStrategyDuringHarvestAndRedeem(2);
      });
    });
  });
});
