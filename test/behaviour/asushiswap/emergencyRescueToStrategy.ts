...
export function shouldAllowEmergencyRescue(): void {
  ...
  it("should allow governace to withdraw LP from uniswap to strategy", async function () {
    ...
    logger.debug(
      `Balance of LP in strategy after withdraw: ${lpBalanceInStrategyAfterWithdraw}`
    );
    logger.debug(
      `Balance of LP in strategy before withdraw: ${lpBalanceInStrategyBeforeWithdraw}`
    );
    ...
    expect(lpBalanceInStrategyAfterWithdraw).eq(stre18);
  });
