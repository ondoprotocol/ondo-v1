...
export function shouldAllowEmergencyRescue(): void {
  ...
  it("should allow governace to withdraw LP from uniswap to strategy", async function () {
    ...
    const lpBalanceInStrategyBeforeWithdraw = await this.poolContract.balanceOf(
      this.strategy.address
    );
    const lpBalanceInStakingBeforeWithdraw = await this.rewardStakingContract.balanceOf(
      this.strategy.address,
      this.slp
    );
    logger.debug(
      `Balance of LP for strategy in staking contract before withdraw: ${lpBalanceInStakingBeforeWithdraw}`
    );
    const encodedFunctionWithdraw = this.rewardStakingContract.interface.encodeFunctionData(
      "withdraw",
      [this.slp, lpBalanceInStakingBeforeWithdraw]
    );
    await this.strategy.multiexcall([
      {
        target: this.rewardStakingContract.address,
        data: encodedFunctionWithdraw,
      },
    ]);
    const lpBalanceInStakingAfterWithdraw = await this.rewardStakingContract.balanceOf(
      this.strategy.address,
      this.slp
    );
    const lpBalanceInStrategyAfterWithdraw = await this.poolContract.balanceOf(
      this.strategy.address
    );
    logger.debug(
      `Balance of LP for strategy in staking contract after withdraw: ${lpBalanceInStakingAfterWithdraw}`
    );

    await provider.send("evm_mine", [vaultInvestedAt + 1000]);
    expect(lpBalanceInStakingAfterWithdraw).eq(0);
    expect(lpBalanceInStrategyBeforeWithdraw).eq(0);
    expect(lpBalanceInStrategyAfterWithdraw).eq(
      lpBalanceInStakingBeforeWithdraw
    );
  });
