...
export function shouldBehaveLikeVaultDuringDeposits() {
  ...
  it("should allow governace to invest using call function", async function () {
    ...
    const callData = this.strategy.interface.encodeFunctionData("invest", [
      this.vaultId,
      ethers.BigNumber.from(this.seniorInvestmentPerInvestor)
        .mul(3)
        .mul(stre18),
      ethers.BigNumber.from(this.juniorInvestmentPerInvestor)
        .mul(3)
        .mul(stre18),
      0,
      0,
    ...
    await expect(
      this.vault
        ...
        .excall(this.strategy.address, callData)
    ).to.be.revertedWith("Unauthorized");

    //let amounts = await this.vault.invest(this.vaultId, 0, 0);
