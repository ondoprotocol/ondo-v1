...
export function shouldBehaveLikeVaultDuringMidtermDeposits(): void {
  ...
  before("should enable tranche tokens", async function () {
    ...
    lpInStrategy = await this.poolContract.balanceOf(this.strategy.address);
    logger.debug(`Before tests lpInStrategy: ${lpInStrategy}`);
  });
  ...
  it("should allow investors to withdraw LP tokens", async function () {
    ...
    let srTrancheBalanceBeforeWithdrawLP = (
      ...
      .toNumber();
    logger.debug(
      `lpBalanceBeforeWithdraw: ${lpBalanceBeforeWithdraw} jrTrancheBalanceBeforeWithdrawLP: ${jrTrancheBalanceBeforeWithdrawLP} srTrancheBalanceBeforeWithdrawLP: ${srTrancheBalanceBeforeWithdrawLP} lpInStrategy: ${lpInStrategy}`
    );
    await this.vault
      .connect(this.signers[0])
      .withdrawLp(this.vaultId, lpInStrategy.div(3)); //as there are 6 investors in total and signer 0 has LP related to 2 investors
    ...
    expect(
      ethers.BigNumber.from(lpInStrategy)
  ...
  it("should allow investors to deposit LP in the Live vault and get back tranche tokens", async function () {
    ...
    let lpBalanceBeforeDeposit = (
      ...
      .div(stre18)
      .toNumber();
    let lpTokensToDeposit = lpInStrategy.div(3);
    ...
    expect(srTrancheBalanceAfterDepositLP).gt(0); //deposited back the LP to get all tranche tokens back i.e., entire investement is back
